...
getModelOutputHexagon<-function(dataIn, modelIn, speciesIn){

  new.data<-dataIn
  hexagonName<-as.character(unique(new.data$Hexagon))
  ...
  write.csv(species.abun.table, file=paste(hexagonFilepath_full, hexagonName,paste(speciesName,hexagonName,"abundance.by.year.csv",sep="."),sep="/"),row.names=FALSE)
  ...
  plot.abun<-ggplot(data=species.abun, aes(x=Year, y=Predicted, ymin=Predicted-SE, ymax=Predicted+SE,group=1))+
    stat_smooth(alpha=0.3,method="lm",col="white")+
    geom_errorbar(width=0, size=1, col="steelblue4")+
    geom_line(color="black")+
    geom_point(stat="identity",size=3,col="steelblue4")+
...
#     scale_color_brewer(palette="Dark2")+
  ...
  abun.plot.out<-plot.abun
  print(abun.plot.out)

  #save plot of abundance estimates
  myFilepath<-paste(hexagonFilepath_full,hexagonName,sep="/")
  ggsave(abun.plot.out, filename=paste(speciesName,hexagonName,"summary.abun.fig.pdf",sep="."),path=myFilepath, width=11,height=8.5, limitsize=FALSE)
