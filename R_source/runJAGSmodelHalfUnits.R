...
runJAGSmodelHalfUnits<-function(data1,data2, mod1, mod2,declineRate,nyrs, nvisits,nsim,scenario,speciesName){
  
  data1<-read.csv("C:/Users/gshriver/Dropbox/ZachGreg/Bayes_PowerAnalysis/Full_Model_8-8-16_GregPC/Data/Data_Half/BMH/BMH_SMI_data_2011-2015_half1.csv",header=TRUE)
  data2<-read.csv("C:/Users/gshriver/Dropbox/ZachGreg/Bayes_PowerAnalysis/Full_Model_8-8-16_GregPC/Data/Data_Half/BMH/BMH_SMI_data_2011-2015_half2.csv",header=TRUE)
  ...
  #get refuge code
  refugeName<-as.character(unique(new.data.1$NWR_code))
  
  ############
  #remove this later

  mod.1<-runUnmarkedNWR(dataIn=data1, speciesName="SALS")
  
  mod.2<-runUnmarkedNWR(dataIn=data2, speciesName="SALS")
  ...
  #get output from abunance models for each of the halves
  new.mod.1<-mod.1
  new.mod.2<-mod.2
  
  #get coefficients from mod.1 and mod.2 (run previously in 'unmarked')
  coefs1<-coef(mod.1)
  coefs2<-coef(mod.2)
  ...
  det.probs.1<-getP(mod.1)
  det.probs.2<-getP(mod.2)
  ...
  for (i in 1:nsim) {
    ...
    data<-switch(scenario,
     "FullPanel"= dataSimFullPanel(dataIn=new.data,modIn=modIn,declineRate=declineRate,nyrs=nyrs,nvisits=nvisits),
     "HalfPanel"= dataSimHalfPanel(dataIn=new.data,modIn=modIn,declineRate=declineRate,nyrs=nyrs,nvisits=nvisits),
