...
runJAGSmodelNWR<-function(dataIn, modIn,declineRate,nyrs, nvisits,nsim,speciesName){
  ...
  for (i in 1:nsim) {
    ...
    dput( list(data=dat, model=res,R=Rmax, mcs=out),
          file=paste(saveFilepath, paste("SMI_Full", "_JAGS_",speciesName,"_",regionName,"_", npoints, "_", declineRate, "decline_", nyrs,"_years",i,".R", sep=''),sep="/")   )
  ...
  str<-c(paste(saveFilepath, paste("SMI_Full", "_JAGS_", speciesName,"_",regionName,"_", npoints, "_", declineRate, "decline_", nyrs,"_years",1:nsim,".R", sep=''),sep="/"))
  truebeta<-c(beta[1:nRefuges],rep(Beta,nRefuges),beta[nRefuges+1], beta[nRefuges+4])

  BetaRes<-array(0, c(nsim, 6, (nRefuges*2+3)))
  ...
  Restab=matrix(0, nrow=(nRefuges*2+3), ncol=5)
  ...
  #replace "Int" with remaining refugeNum
  missing.ref<-as.character(setdiff(ref.table$refugeNum,ref.table.1$refugeNum))

  ref.table.1$refugeNum[is.na(ref.table.1$refugeNum)] <-missing.ref
  ref.table.out.1<-na.omit(ref.table.1)
  ref.table.out<-rbind(ref.table.out.1[nRefuges,],ref.table.out.1[1:(nRefuges-1),])
  ref.table.out<-data.frame(NWR_code=ref.table.out$NWR_code, refugeNum=ref.table.out$refugeNum)

  #format results as table
  abun.est<-Restab[2:(nRefuges+1),]
  ...
  write.csv(abun.est.out, file=paste(saveFilepath, paste("SMI_Full", speciesName, regionName, "JAGS.abundance.output.csv",sep="_"),sep="/"))

  power.table<-Restab[(nRefuges+2):(nRefuges*2+1),]
