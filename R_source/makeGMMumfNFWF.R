...
makeGMMumfNFWF<-function(dataIn,species,distanceCode){
  require(unmarked)
  ...
  # data.count.melt <- melt(data.new, id=c("PointID","AlphaCode","DistCode","VisitNum","Year","Year.Unit","Year.Point","Year.Point.Visit","State","RegionNum","NWR","SiteID","SiteIDnum","Hexagon","Longitude","Latitude"),
  #                         measure=c("Min.1","Min.2","Min.3","Min.4","Min.5"), na.rm=FALSE)
  
  #temporary!! until Lat/Long and others come in.
  data.count.melt <- melt(data.new, id=c("PointID","AlphaCode","DistCode","VisitNum","Year","Year.Unit","Year.Point","Year.Point.Visit","State","RegionNum","NWR","ProjectID","SiteID","SiteIDnum","Hexagon"),
                          measure=c("Min.1","Min.2","Min.3","Min.4","Min.5"), na.rm=FALSE)
  
  
  #cast count data (one visit at first) add try()
  count.data <- cast(data.count.melt, Year.Point.Visit ~ variable, add.missing=TRUE,fun.aggregate=sum, subset=c(AlphaCode==species & DistCode==0))
  head(count.data)
  ...
  head(data.new)
  # data.new.covs<-data.new[,c("Year.Point.Visit","State","RegionNum","NWR","SiteID","SiteIDnum","Hexagon", "Longitude","Latitude",
  #                            "Observer","Month","Day","time.scale","temp.scale","Tide")]

  #temp data.new.covs
  data.new.covs<-data.new[,c("Year.Point.Visit","State","RegionNum","NWR","ProjectID","SiteID","SiteIDnum","Hexagon", 
                             "Observer","Month","Day","time.scale","temp.scale","Tide")]
...
#   write.csv(count.data.out,paste(speciesUMFdataFilepath,paste(species,refugeName,"100_long.csv",sep="."),sep="/"),row.names=FALSE)
  ...
  # covs<-species.data[,c("Year","State","RegionNum","NWR","SiteID","SiteIDnum","Hexagon","VisitNum","Month","Day", "Longitude","Latitude",
  #                       "Observer","time.scale","temp.scale","Tide")]
  #temp covs
  covs<-species.data[,c("PointID","Year","State","RegionNum","NWR","ProjectID","SiteID","SiteIDnum","Hexagon","VisitNum","Month","Day",
                        "Observer","time.scale","temp.scale","Tide")]
  
  
  covs$SiteIDnum<-as.factor(as.character(covs$SiteIDnum))
