...
dataSimUnits_triennial<-function(dataIn,modIn,declineRate,nyrs,nvisits){
  ...
  sim.cell.out<-list()
   ...
   for(q in 1:length(unitNumList)){
    ...
    for(j in 1:nyrs){
      ...
      det.2[[j]]<-data.frame(matrix(as.numeric(det.1),ncol=5)) #5 1-min intervals during surveys
      det.out<-det.2
    }
  ...
  det.out.3<-matrix(unlist(new.out),ncol=5)
  det.out.4[[v]]<-det.out.3
  }

cell.2=sort(seq(1:npoints))

#generate sequence of every third number 1 through n
grab.years<-c(seq(1,nyrs,3))

#select every other column
sim.N.out.3<-sim.N.out.2[,grab.years]

det.out.5<-det.out.4[grab.years]


return(list(detList=det.out.5, covList=covList, N=sim.N.out.3, cell=cell.2))
