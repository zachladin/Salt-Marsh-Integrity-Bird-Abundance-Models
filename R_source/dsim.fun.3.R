...
dsim.fun.3<-function(dataIn,modIn,declineRate,nyrs,nvisits){
  ...
  sim.N.out.2<-data.frame(matrix(unlist(sim.N.out),ncol=5))
  ...
  det.out.2<-data.frame(matrix(unlist(new.sim.det.out),ncol=5))

  cell.2=sort(seq(1:npoints))

  return(list(detList=new.sim.det.out, covList=covList, N=sim.N.out, cell=cell.2))
