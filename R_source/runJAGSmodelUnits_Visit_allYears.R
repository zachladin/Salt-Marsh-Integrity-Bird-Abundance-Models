...
runJAGSmodelUnits_Visit_allYears<-function(dataIn, modIn,declineRate,nyrs, nvisits,nsim,speciesName,filePath,scenario){
  ...
  dir.create(paste(filePath,speciesName,sep="/"))
  dir.create(paste(filePath,speciesName,refugeName,sep="/"))
  dir.create(paste(filePath,speciesName,refugeName,paste(scenario,"Power","_", speciesName,"_",refugeName,"_",declineRate, "decline_",nyrs,"years_",nvisits,"visits" ,sep=''),sep="/"))
  ...
  for (i in 1:nsim) {
    ...
    #########################################################################################
    #simulate data
    if(scenario=="Full_annual" | scenario=="Half_A_annual" | scenario=="Half_B_annual" | scenario=="Third_A_annual" | scenario=="Third_B_annual" | scenario=="Third_C_annual"){
      dat<-dataSimUnits_annual(dataIn=new.data,modIn=mod.1,declineRate=declineRate,nyrs=nyrs,nvisits=nVisits)
    }else{
      if(scenario=="Full_biennial" | scenario=="Half_A_biennial" | scenario=="Half_B_biennial" | scenario=="Third_A_biennial" | scenario=="Third_B_biennial" | scenario=="Third_C_biennial"){
        dat<-dataSimUnits_biennial(dataIn=new.data,modIn=mod.1,declineRate=declineRate,nyrs=nyrs,nvisits=nVisits)
      }else{
        if(scenario=="Full_triennial" | scenario=="Half_A_triennial" | scenario=="Half_B_triennial" | scenario=="Third_A_triennial" | scenario=="Third_B_triennial" | scenario=="Third_C_triennial" ){
          dat<-dataSimUnits_triennial(dataIn=new.data,modIn=mod.1,declineRate=declineRate,nyrs=nyrs,nvisits=nVisits)
          }
        }
      }
    ...
    if(scenario=="Full_annual" | scenario=="Half_A_annual" | scenario=="Half_B_annual" | scenario=="Third_A_annual" | scenario=="Third_B_annual" | scenario=="Third_C_annual"){
     sampYears<-nyrs
      ...
      if(scenario=="Full_biennial" | scenario=="Half_A_biennial" | scenario=="Half_B_biennial" | scenario=="Third_A_biennial" | scenario=="Third_B_biennial" | scenario=="Third_C_biennial"){
        sampYears<-length(c(seq(1,nyrs,2)))
      }else{
        if(scenario=="Full_triennial" | scenario=="Half_A_triennial" | scenario=="Half_B_triennial" | scenario=="Third_A_triennial" | scenario=="Third_B_triennial" | scenario=="Third_C_triennial" ){
          sampYears<-length(c(seq(1,nyrs,3)))
        }
      }
    }
    ...
    #bundle data
    data <- list(y = y, M = nrow(y), nUnits=nUnits, nYears=sampYears,  nVisits=nVisits, unit=newUnit, visit=newVisit)
    ...
    # MCMC settings
    nc <- 3   ;   ni <- 30000  ;   nb <- 10000  ;   nt <- 20 #ni=30000, nb=5000
    ...
    saveFilepath<-paste(filePath,speciesName,refugeName,paste(scenario, "Power","_",speciesName,"_",refugeName ,"_",declineRate, "decline_",nyrs,"years_",nvisits,"visits", sep=''),sep="/")
  ...
  #coefs.temp<-c(beta0, beta1, beta2,beta3, beta[(length(unitNumList)+1):(length(unitNumList)+4)])
  ...
  abun.est<-data.frame(abun.est, CI[1:nUnits,])
  abun.est.1<-cbind(ref.table.out, abun.est)
  abun.est.out<-abun.est.1[,c("SMI_Unit","unitNum","numPoints","Mean","rmse","BiasMean","Predicted","X2.5.","X97.5.")]
  colnames(abun.est.out)[8]<-"abun_lower2.5%"
  colnames(abun.est.out)[9]<-"abun_upper97.5%"
  ...
  power.table<-Restab[(nUnits+1):(nUnits*2),]
  power.table<-data.frame(power.table, CI[(nUnits+1):(nUnits*2),])
  ...
  power.table.1<-data.frame(power.table,abun.est.out[,c("SMI_Unit","unitNum","Predicted","abun_lower2.5%","abun_upper97.5%")])
  power.table.out<-power.table.1[,c("SMI_Unit","unitNum","NumPoints","nVisits","Predicted","abun_lower2.5.","abun_upper97.5.","Mean","X2.5.","X97.5.","rmse","BiasMean","CI","significance","pwr","model_Type")]
  colnames(power.table.out)[5]<-"Predicted.abun"
  colnames(power.table.out)[6]<-"abun_lower2.5%"
  colnames(power.table.out)[7]<-"abun_upper97.5%"
  colnames(power.table.out)[8]<-"Mean_gamma"
  colnames(power.table.out)[9]<-"gamma_lower2.5%"
  colnames(power.table.out)[10]<-"gamma_upper97.5%"
