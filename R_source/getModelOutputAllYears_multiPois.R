...
getModelOutputAllYears_multiPois<-function(dataIn, modelIn, speciesName){
  ...
  write.csv(species.abun.table, file=paste(getwd(), "Results", speciesName,refugeName,paste(speciesName,refugeName,"abundance.by.unit_4-07-2020.csv",sep="."),sep="/"),row.names=FALSE)

  #save detection probs
  det.probs<-unique(getP(modelIn))
  write.csv(det.probs,file=paste(getwd(), "Results", speciesName,refugeName,paste(speciesName,refugeName,"detection.by.visit_4-07-2020.csv",sep="."),sep="/"),row.names=FALSE)
  ...
  #now save for later
  write.csv(mod.coefs.df.2, file=paste(getwd(), "Results", speciesName,refugeName,paste(speciesName,refugeName,"coefficients.allYears.by.unit_4-07-2020.csv",sep="."),sep="/"),row.names=FALSE)
  ...
  species.abun$SMI_Unit<-factor(species.abun$SMI_Unit, levels=rev(unique(species.abun$SMI_Unit)))
  ...
  ggsave(plot.abun, filename=paste(speciesName,refugeName,"unit.summary.abun.fig_4-07-2020.pdf",sep="."),path=myFilepath, width=6,height=7, limitsize=FALSE)
  ggsave(plot.abun, filename=paste(speciesName,refugeName,"unit.summary.abun.fig_4-07-2020.png",sep="."),path=myFilepath, width=6,height=7, limitsize=FALSE)
