...
runJAGSmodelUnits_Visit_Observer<-function(dataIn, modIn,declineRate,nyrs, nvisits,nsim,speciesName){
  ...
  for (i in 1:nsim) {
    ...
    # MCMC settings
    nc <- 3   ;   ni <- 5000  ;   nb <- 1000  ;   nt <- 20

    ############################################
    #run with 'rjags'
    mod<-jags.model("JAGS.model.unit.txt", data, inits, n.chains = nc, n.adapt=500)
    ...
    dir.create(paste(getwd(),"Results","SMI_Full","Power_Bayes",speciesName,refugeName,paste("SMI_Full_","Power","_", speciesName,"_",refugeName,"_",declineRate, "decline_",nyrs,"_years" ,sep=''),sep="/"))
    saveFilepath<-paste(getwd(),"Results","SMI_Full","Power_Bayes",speciesName,refugeName,paste("SMI_Full_", "Power","_",speciesName,"_",refugeName ,"_",declineRate, "decline_",nyrs,"_years", sep=''),sep="/")

    dput( list(data=dat, model=res,R=Rmax, mcs=out),
          file=paste(saveFilepath, paste("JAGS_",speciesName,"_",refugeName,"_", npoints, "_", declineRate, "decline_", nyrs,"_years",i,".R", sep=''),sep="/")   )
  ...
  beta.use<-beta[1:length(unitNumList)]
  ...
  coefs<-if(length(unitNumList)==1){
    c(beta0, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==2){
    c(beta0, beta1, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==3){
    c(beta0, beta1, beta2, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==4){
    c(beta0, beta1, beta2,beta3, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==5){
    c(beta0, beta1, beta2,beta3,beta4, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==6){
    c(beta0, beta1, beta2,beta3,beta4,beta5, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==7){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==8){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==9){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7,beta8, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==10){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==11){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==12){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==13){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==14){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==15){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==16){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==17){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==18){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16,beta17, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==19){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16,beta17,beta18, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==20){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16,beta17,beta18,beta19, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==21){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16,beta17,beta18,beta19,beta20, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==22){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16,beta17,beta18,beta19,beta20,beta21, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==23){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16,beta17,beta18,beta19,beta20,beta21,beta22, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==24){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16,beta17,beta18,beta19,beta20,beta21,beta22,beta23, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }else{if(length(unitNumList)==25){
    c(beta0, beta1, beta2,beta3,beta4,beta5,beta6,beta7, beta8,beta9,beta10,beta11,beta12,beta13,beta14,beta15,beta16,beta17,beta18,beta19,beta20,beta21,beta22,beta23,beta24, beta[(length(unitNumList)+1):(length(unitNumList)+obsCount+4)])
  }}}}}}}}}}}}}}}}}}}}}}}}}
  ...
  str<-c(paste(saveFilepath, paste("JAGS_", speciesName,"_",refugeName,"_", npoints, "_", declineRate, "decline_", nyrs,"_years",1:nsim,".R", sep=''),sep="/"))
  ...
  Restab=matrix(0, nrow=(nUnits*2+2), ncol=5)
  ...
  ref.table.out<-data.frame(SMI_Unit=ref.table.1$SMI_Unit, unitNum=ref.table.1$unitNum)
